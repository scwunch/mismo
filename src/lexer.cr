...
class Lexer
  ...
  def prev_token
    @prev_token || Token::BeginFile.new(Location.zero)
  end

  def initialize(@reader : Reader, @log : Logger, @out : Lexer::Out)
  end
  def initialize(text : String, @log : Logger, @out : Lexer::Out)
    @reader = Reader.new(text)
  end

  module Lexer::Out
    abstract def emit(token : Token)
    abstract def signal_end_of_file
    abstract def send_log(log : Logger)
    abstract def import(import_loc : Location, path : String, import_alias : String?, import_bindings : Array({String, String})?)
  end
  ...
  def tokenize
    @log.info_descend(Location.zero, "Lexer.tokenize start") do
      lex_body
    end
    # Push any remaining token in the buffer (the very last token)
    if last_tok = @prev_token
        @out.emit(last_tok)
        @prev_token = nil # Clear it after emitting
    end
    @log.info(@reader.location, "Lexer.tokenize end")
    # signal_full_declaration # This seems to be called after certain keywords, not just at the end.
    @out.signal_end_of_file # This should emit an EOF token.
    @out
  end

  # In push_token, the previous token is emitted, and the new token is stored.
  # This means the last token is only emitted when the *next* token is pushed, or at EOF.
  def push_token(tok : Token? = nil)
    @log.debug(@reader.location, "pushing token: #{tok}")
    if prev = @prev_token
      @out.emit(prev)
    end
    @prev_token = tok
  ...
  def signal_full_declaration
    # This was `out.send_log(log = log.clone())`
    # Assuming Lexer::Out has send_log and Logger has clone
    @out.send_log(@log.clone)
  end

  def lex_body
    @log.debug(@reader.location, "lex_body")
    while true
      loc = @reader.location
  ...
  def push_word_operator(loc, op)
    @log.debug_descend(loc, "push word operator: #{op}") do
      if op == Operator::Not && (prev = @prev_token) && prev.data == Operator::Is
        @prev_token = Token.operator(prev.location, Operator::IsNot)
        @log.debug(loc, "pushed 'is not' operator")
      elsif op == Operator::In && (prev = @prev_token) && prev.data == Operator::Not
        @prev_token = Token.operator(prev.location, Operator::NotIn)

class Specializer
  ...
  def initialize(@env : TypeEnv, main_def : FunctionDef)
    @env = env
    get_or_make_function(Location.zero, main_def, Slice(Type).empty, 0)
  end
  ...
  def run
    ...
    while function = queue.shift?
      raise "WAHT" if function.name.starts_with?("__zig_")
      body = type_checker.type_check_function(function)
      if body.nil?
        next
      end
      function.body = body
  ...
  # If the function already exists, just return it.  Otherwise, insert it into the map and
  # queue it for processing (unless it is an extern function, in which case there is no need
  # to process it)
